 ...
 **************************************************************/
// Initialize InputManager in initialize() function
// handleKeyDown logic has been moved to js/inputManager.js
...
async function initialize() { // Made async
    ...
    if (window.ToastUIManager) {
        ...
        if (window.InputManager) {
            window.inputManager = new window.InputManager(window.gameState, window.audioManager, window.mapRenderer, window.turnManager, window.interaction, window.combatManager, window.inventoryManager);
            window.inputManager.initialize();
            logToConsole("InputManager initialized.", "info");
        } else {
            console.error("SCRIPT.JS: InputManager class not available.");
        }


        requestAnimationFrame(gameLoop); // Start the main game loop

        // document.addEventListener('keydown', handleKeyDown); // Handled by InputManager now
