...
// The AssetManager is designed to be used by both the game engine and the map editor
// to ensure a single source of truth for all assets.
//
// For the Map Editor:
// 1. Initialization:
//    - Create an instance of AssetManager: `const editorAssetManager = new AssetManager();`
//    - Load definitions: `await editorAssetManager.loadDefinitions();`
//      This will provide the editor with all available tiles, items, NPCs, etc.
//      The editor should handle potential errors during loadDefinitions, especially for UGC.
//
// 2. Populating Palettes:
//    - Tiles: Iterate `Object.entries(editorAssetManager.tilesets)` to build the tile palette.
//      Each entry gives [tileId, tileDefinition]. The editor would use tileDefinition.sprite (or similar)
//      and tileDefinition.name for display.
//    - Items/NPCs: Use `editorAssetManager.itemsById` and `editorAssetManager.npcsById`
//      to list placeable entities (e.g., for drag-and-drop placement).
//
// 3. Loading Existing Maps for Editing:
//    - Use `await editorAssetManager.loadMap(mapIdToEdit);`
//    - The editor can then display `editorAssetManager.currentMap` for modification.
//      The `loadMap` method will attempt to load from user_assets first, then base assets.
//
// 4. Saving Maps:
//    - The editor should construct a map JSON object matching the defined structure
//      (see ASSET_STRUCTURE.md or example map files). This includes:
//      `id`, `name`, `dimensions`, `layers` (e.g., `ground`, `objects`), `npc_spawns`, etc.
//    - Tile IDs used in the map layers *must* correspond to IDs present in `editorAssetManager.tilesets`.
//      The editor should validate this or only allow selection from the loaded tilesets.
//    - Saved maps should be written to `/assets/maps/your_map_id.json` or
//      `/user_assets/maps/your_map_id.json`. The path structure (e.g. subdirectories for categories)
//      should be consistent with how `mapIndex.json` and `loadMap` expect to find them.
//    - Map IDs should be unique. If saving a map with an ID that already exists, the editor
//      should confirm overwrite.
//    - After saving a new map, especially to base assets, the corresponding `mapIndex.json`
//      (`/assets/maps/mapIndex.json` or `/user_assets/maps/mapIndex.json`) needs to be updated
//      to include the new map's `id` and `name` for it to appear in the game's map selector.
//      The editor could automate this or remind the user.
//
// 5. Handling Asset Updates:
//    - If asset definitions (tiles, items, NPCs) are modified while the editor is running,
//      it might need to re-call `editorAssetManager.loadDefinitions()` and refresh its palettes
//      and potentially the currently loaded map display.
//
// By using the same AssetManager and asset files, consistency between the editor
// and the game is maintained.
