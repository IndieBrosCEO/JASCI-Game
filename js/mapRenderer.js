...
window.mapRenderer = {
    ...
    // Note: gameState.mapLevels and gameState.playerPos (including .z) 
    // and gameState.currentViewZ will be updated in script.js (or its own map change handlers)
...
renderMapLayers: function () {
    ...
    // Targeting Cursor (on current Z-level for now)
    if (gameState.isTargetingMode && tileCacheData) {
        // TODO: Targeting Z needs to be considered. If targeting another Z, cursor shouldn't show here.
        // For now, assume targeting is on currentViewZ.
...
getCollisionTileAt: function (x, y) { // This needs to be Z-aware: getCollisionTileAt(x,y,z)
    const mapData = this.getCurrentMapData();
    // This needs to look at mapData.levels[z]
    if (!mapData || !mapData.layers) return ""; // Needs update to mapData.levels

    const tilesets = assetManagerInstance?.tilesets;
    if (!tilesets) return "";

    // This logic is now insufficient as it doesn't consider Z.
    // It needs to check layers within the specific Z-level.
    // For now, this function is not Z-aware and will break/be inaccurate.
