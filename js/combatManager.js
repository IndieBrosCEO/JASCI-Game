class CombatManager {
    ...
    shareAggroWithTeam(damagedEntity, attacker, threatAmount) {
        ...
        this.gameState.npcs.forEach(npc => {
            if (npc.teamId === teamId && npc !== attacker && npc !== damagedEntity) {
                ...
                // logToConsole(`INFO: ${npc.name} threat vs ${attacker === this.gameState ? 'Player' : (attacker.name || attacker.id)} updated.`, 'gold');
            }
        });
        if (this.gameState.player.teamId === teamId && attacker !== this.gameState && damagedEntity !== this.gameState) {
            ...
            // logToConsole(`INFO: Player threat vs ${attacker.name || attacker.id} updated.`, 'lightgreen');
        }
    }
    ...
    async nextTurn(previousAttackerEntity = null) {
        ...
        if (attacker?.statusEffects) {
            ...
            for (const effectId in attacker.statusEffects) {
                ...
                if (effect.damagePerTurn && effect.damageType) {
                    ...
                    if (partToDamage) {
                        const DPT = effect.damagePerTurn; // Assume damage is already rolled or fixed
                        ...
                        if (partToDamage.current <= 0) {
                            ...
                            if (currentEntry.isPlayer) { this.endCombat(); window.gameOver(this.gameState); return; }
                                ...
                                await this.nextTurn(attacker); return; // Effectively skips defeated NPC's actions
                            }
                        }
                    }
                }
        ...
        if (this.gameState.environmentalEffects?.smokeTiles) {
            ...
            for (let i = this.gameState.environmentalEffects.smokeTiles.length - 1; i >= 0; i--) {
                this.gameState.environmentalEffects.smokeTiles[i].duration--;
                if (this.gameState.environmentalEffects.smokeTiles[i].duration <= 0) {
                    this.gameState.environmentalEffects.smokeTiles.splice(i, 1); changed = true;
                    logToConsole(`Smoke dissipates at (${this.gameState.environmentalEffects.smokeTiles[i]?.x}, ${this.gameState.environmentalEffects.smokeTiles[i]?.y}).`, 'grey');
                }
            }
            if (changed && window.mapRenderer) window.mapRenderer.scheduleRender();
            this.initiativeTracker.map(e => e.entity).forEach(c => {
                if (!c || (!c.mapPos && c !== this.gameState)) return;
                const pos = (c === this.gameState) ? this.gameState.playerPos : c.mapPos; if (!pos) return;
                const onSmoke = this.gameState.environmentalEffects.smokeTiles.some(s => s.x === pos.x && s.y === pos.y);
                const currentStatus = c.statusEffects ? c.statusEffects["in_smoke"] : null;
                if (onSmoke) {
                    if (!currentStatus) { if (!c.statusEffects) c.statusEffects = {}; c.statusEffects["in_smoke"] = { id: "in_smoke", displayName: "In Smoke", duration: 1, sourceItemId: "smoke_grenade_thrown", description: "Vision obscured. Attack -2." };}
                    else currentStatus.duration = Math.max(currentStatus.duration, 1);
                } else if (currentStatus) delete c.statusEffects["in_smoke"];
            });
        }
        ...
        if (currentEntry.isPlayer) {
            ...
            this.gameState.combatCurrentDefender = this.gameState; // Default NPC target is player
            this.gameState.defenderMapPos = this.gameState.playerPos ? { ...this.gameState.playerPos } : null;
            // logToConsole(`NPC ${attackerName} targets Player.`, 'gold');
        }
    ...
    handleConfirmedAttackDeclaration() {
        ...
        if (!this.gameState.combatCurrentDefender) { logToConsole("ERROR: No target selected for attack.", 'red'); this.promptPlayerAttackDeclaration(); return; }
        const fireMode = document.getElementById('combatFireModeSelect')?.value || "single";
        const bodyPart = bodyPartSelect.value;
        this.gameState.pendingCombatAction = { target: this.gameState.combatCurrentDefender, weapon: weaponObj, attackType, bodyPart, fireMode, actionType: "attack", entity: this.gameState, skillToUse: null, targetTile: null, actionDescription: `${weaponObj ? weaponObj.name : "Unarmed"} attack on ${this.gameState.combatCurrentDefender.name}'s ${bodyPart}` };
        logToConsole(`Player declares: ${attackType} attack on ${this.gameState.combatCurrentDefender.name}'s ${bodyPart} with ${weaponObj ? weaponObj.name : 'Unarmed'} (Mode: ${fireMode}).`, 'lightgreen');
        if (weaponObj?.type === "weapon_thrown_explosive" || (weaponObj?.type === "weapon_thrown_utility" && weaponObj.id === "smoke_grenade_thrown")) { // Smoke needs a target tile too
            this.gameState.pendingCombatAction.skillToUse = weaponObj?.type === "weapon_thrown_explosive" ? "Explosives" : "Strength"; // Or another skill for utility throws
            if (this.gameState.combatCurrentDefender.mapPos) this.gameState.pendingCombatAction.targetTile = { ...this.gameState.combatCurrentDefender.mapPos };
            else { logToConsole("ERROR: Target for thrown item has no mapPos.", 'red'); this.promptPlayerAttackDeclaration(); return; }
            logToConsole(`Throwing ${weaponObj.name} at tile (${this.gameState.pendingCombatAction.targetTile.x},${this.gameState.pendingCombatAction.targetTile.y}).`, 'lightgreen');
        } else {
    ...
    handleDefenderActionPrompt() {
        ...
        if (!defender && !(this.gameState.pendingCombatAction?.targetTile && (this.gameState.pendingCombatAction?.weapon?.type === "weapon_thrown_explosive" || this.gameState.pendingCombatAction?.weapon?.id === "smoke_grenade_thrown"))) {
    ...
    calculateAttackRoll(attacker, weapon, targetBodyPartArg, actionContext = {}) {
        ...
        if (attacker.statusEffects) {
            if (attacker.statusEffects["blinded_pepper_spray"]?.visionPenalty) statusEffectAttackPenalty += attacker.statusEffects["blinded_pepper_spray"].visionPenalty;
            if (attacker.statusEffects["irritated_tear_gas"]?.accuracyPenalty) statusEffectAttackPenalty += attacker.statusEffects["irritated_tear_gas"].accuracyPenalty;
            if (attacker.statusEffects["in_smoke"] && lightingPenalty === 0) statusEffectAttackPenalty += -2; // Only if not already dark
        }
        ...
        return { roll: totalAttackRoll, naturalRoll: baseRoll, isCriticalHit: canCrit && baseRoll === 20, isCriticalMiss: canCrit && baseRoll === 1, lightingPenaltyApplied: lightingPenalty };
    }
    ...
    applySpecialEffect(attacker, item, targetEntity = null, impactTile = null) {
        ...
        if (burstRadiusTiles > 0 && impactTile) {
            ...
            if (effectString === "Creates smoke screen" && item.id === "smoke_grenade_thrown" && !areaEffectProcessedThisCall) {
        ...
        affectedEntities.forEach(entity => {
            ...
            if (effectString === "Creates smoke screen" && item.id === "smoke_grenade_thrown") {
                existingEffect = entity.statusEffects["in_smoke"]; duration = 2; // Initial status if caught in burst
                ...
                existingEffect = entity.statusEffects["irritated_tear_gas"]; duration = 3; penalty = -4;
    ...
    async processAttack() {
        ...
        if (attackType === 'melee' && window.animationManager && defender) {
            ...
            if (weapon.id === 'smoke_grenade_thrown') cloudParams.particleColor = 'grey';
                ...
                cloudParams.coneDirection = this.gameState.defenderMapPos || defender?.mapPos || { x: cloudParams.centerPos.x + 1, y: cloudParams.centerPos.y };
                cloudParams.maxRadius = 1.5; cloudParams.duration = 1500; cloudParams.activeSpawningDuration = 500; cloudParams.particleLifetime = 1000; cloudParams.particleColor = 'orange'; cloudParams.particleSpriteOptions = ['.', ':', ';']; cloudParams.coneAngle = Math.PI / 6; cloudParams.spawnRate = 20;
            }
        ...
        if (attackType === 'ranged' && weapon) {
            ...
            if (attackerMapPos && targetMapPos) {
                ...
                actionContext.isGrappling = attacker.statusEffects?.isGrappled && attacker.statusEffects.grappledBy === (defender === this.gameState ? 'player' : defender?.id); // Check if attacker is grappling this defender
        ...
        logMsg = `ATTACK: ${attackerName} targets ${defender ? defenderName + "'s " + intendedBodyPart : "tile at " + (this.gameState.pendingCombatAction?.targetTile?.x || '?') + "," + (this.gameState.pendingCombatAction?.targetTile?.y || '?')} with ${weapon ? weapon.name : 'Unarmed'} (Mode: ${fireMode}). ` +
                 ...
                 `BodyPart: ${actionContext.bodyPartModifier}, Range: ${actionContext.rangeModifier}, Mode: ${actionContext.attackModifier}, Move: ${actionContext.attackerMovementPenalty}, Light: ${attackResult.lightingPenaltyApplied}, Status: ${actionContext.statusEffectAttackPenalty || 0})`;
        ...
        } else if (weapon?.type === "weapon_thrown_explosive" || (weapon?.type === "weapon_thrown_utility" && weapon.id === "smoke_grenade_thrown")) {
            hit = true;
                ...
                this.getCharactersInBlastRadius(determinedImpactTile, burstRadiusTiles).forEach(char => {
                    ...
                    if (isThrownExplosive && (char !== defender || (char === defender && !hit))) {
                        if ((rollDie(20) + getStatModifier("Dexterity", char)) >= attackResult.roll) {
                            affectedByBlast = false; logToConsole(`${char.name || "Player"} dodged blast!`, 'lightgreen');
                        } else logToConsole(`${char.name || "Player"} failed to dodge blast.`, 'orange');
                    }
    ...
    getCharactersInBlastRadius(impactTile, burstRadiusTiles) {
        ...
        checkEntity(this.gameState, this.gameState.playerPos); // Check player
        this.gameState.npcs.forEach(npc => checkEntity(npc, npc.mapPos)); // Check NPCs
    ...
    applyDamage(attacker, entity, bodyPartName, damageAmount, damageType, weapon, bulletNum = 0, totalBullets = 0) {
        ...
        this.shareAggroWithTeam(entity, attacker, damageAmount); // Share aggro based on raw damage before armor
    ...
    _npcSelectTarget(npc) {
        ...
        if (closestTarget) { this.gameState.combatCurrentDefender = closestTarget; this.gameState.defenderMapPos = closestTarget === this.gameState ? { ...this.gameState.playerPos } : { ...closestTarget.mapPos };
    ...
    async executeNpcCombatTurn(npc) {
        ...
        for (let iter = 0; !turnEnded && (npc.currentActionPoints > 0 || npc.currentMovementPoints > 0) && iter < 10; iter++) {
            ...
            if (canAttack && npc.currentActionPoints > 0) {
                ...
                else { logToConsole(`NPC ACTION: ${npcName} cannot reach target for melee.`, 'orange'); turnEnded = true; } // If can't move closer, end.
            } else if (npc.currentMovementPoints > 0) { // No attack, but can move
                 if (await this.moveNpcTowardsTarget(npc, currentTargetPos)) actionTaken = true; // Try to move closer
                 else { logToConsole(`NPC ACTION: ${npcName} has no clear action and cannot move closer.`, 'orange'); turnEnded = true; }
            } else { // No AP and no MP
                turnEnded = true;
            }
            if (!actionTaken && !(npc.currentActionPoints > 0 && canAttack)) turnEnded = true; // If no action was possible and can't attack
